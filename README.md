...
`apps/ai/baseline/` contains the V3 architecture: **BasicVSR with a SPyNet optical-flow backbone** (sequence length 15, 4x upscale). `vsr_inference.py` is the single inference module: it exposes `VSRInferenceEngine`, which slides a 15-frame window over the video one frame at a time (bounded RAM, one forward pass per output frame), with progress callbacks and cooperative cancellation. Training artifacts live in `Model_v3.ipynb`. Inference parameters are owned by the engine/checkpoint — the backend does not override them.
