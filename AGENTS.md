...
3. On the AI's `completed` line: in `path` mode the backend verifies the output file the AI wrote; in `remote` mode it first downloads `GET /result/:jobId` from the AI and saves it locally. Either way the local result is at `storage/results/{jobId}_enhanced.mp4` (always H.264 — the AI re-encodes its raw mp4v output for browser playback) plus a best-effort `{jobId}_original.mp4` comparison video, and the job is marked `completed`. The final enhanced video only ever becomes available at this point — during processing the frontend plays the sampled preview frames as a buffered JPEG "flipbook" (delayed-broadcast style, paced by `fps`/`stride`), which never substitutes for the completed result.
