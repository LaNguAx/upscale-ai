...
- `apps/ai` — Python FastAPI + PyTorch BasicVSR/SPyNet inference service (port 8000). Exposes `/health`, path-based `/process`, multipart `/process-upload`, `/result/:jobId` (+ `/result/:jobId/original`), `/cancel`, and `/preview/:jobId/latest` + `/preview/:jobId/:frameIndex` (with `_in` file keys for the matching original frames; token-guarded internal endpoints). Results are re-encoded to browser-safe H.264 MP4 via ffmpeg (required on PATH). Managed via `requirements.txt`; the `package.json` only wraps uvicorn for Turborepo. Run `pnpm --filter ai run setup` once (the explicit `run` matters — pnpm 10 otherwise invokes its builtin `setup` command) to install Python deps. See `apps/ai/AGENTS.md`.
...
- `@repo/schemas` — Zod 4 schemas and inferred types. Subpaths: `/health`, `/jobs` (`JobState`, `JobStatus`, `JobUpdate`, `isTerminalJobState`, `jobErrorCodeSchema`/`JobErrorCode` — currently just `INPUT_RESOLUTION_TOO_HIGH` — surfaced as the optional `errorCode` on `JobStatus`/`JobUpdate` so the client renders its own copy instead of raw error text, `jobPreviewSchema` incl. optional `originalImageUrl` and flipbook pacing metadata `fps`/`stride`), `/upload` (`UploadResponse`, `JobResult`, `CancelJobResponse`), `/errors` (RFC 7807 `problemDetailsSchema`).
...
- The backend→AI transport is selected by `AI_TRANSFER_MODE` (see `apps/backend/AGENTS.md`):
  ...
  - Progressive preview frames are proxied cache-through by the backend: `GET /api/upload/preview/...` serves from `storage/previews` when cached, otherwise fetches that exact frame over HTTP from the AI's `/preview/:jobId/...` endpoints while the job is processing — in **both** transports (there is no filesystem shortcut, even in `path` mode). The frontend only ever sees the backend's own `/api/upload/preview/...` URLs, never the AI service's address.
- Job state lives in an in-memory `Map` in `UploadService` — it does not survive a backend restart, and there is no cleanup of old uploads/results (known limitations). Cached preview frames are the exception: both the backend (`storage/previews/{jobId}`) and the AI delete a job's preview directory once it reaches a terminal state.
...
2. Backend checks AI `GET /health` (requires `model_loaded: true`), then hands off work per `AI_TRANSFER_MODE`: in `path` mode `POST /process` with `{ jobId, inputPath, outputPath, scale: 4 }`; in `remote` mode `POST /process-upload` with the `video` file (multipart). Either way it consumes the NDJSON progress stream and pushes each update to the per-job SSE stream (`/api/upload/events/:jobId`). Along the way, sampled original+enhanced preview JPEG pairs are written on the AI (frame 1, then every `PREVIEW_EVERY_N_FRAMES`th frame — default 2, dense enough for flipbook playback) and referenced on `processing` lines with `fps`/`stride` pacing metadata; the backend records that metadata and surfaces it as an optional `preview` object (`jobPreviewSchema`, with public `/api/upload/preview/:jobId/:frameIndex` and `…/original` URLs) on `JobUpdate`/`JobStatus`. The JPEGs themselves are fetched lazily by the backend's cache-through preview proxy when the client requests them (cached under `PREVIEW_DIR/{jobId}/`); a preview fetch failure surfaces as a 404/502 on that frame and never fails the job.
3. On the AI's `completed` line: in `path` mode the backend verifies the output file the AI wrote; in `remote` mode it first downloads `GET /result/:jobId` from the AI and saves it locally. Either way the local result is at `storage/results/{jobId}_enhanced.mp4` (always H.264, with the original input's audio muxed in as AAC when present — the AI re-encodes its raw mp4v output for browser playback) plus a best-effort `{jobId}_original.mp4` comparison video (audio-free — it plays muted under the divider), and the job is marked `completed`. The final enhanced video only ever becomes available at this point — during processing the frontend plays the sampled preview frames as a buffered JPEG "flipbook" (delayed-broadcast style, paced by `fps`/`stride`), which never substitutes for the completed result.
4. Cancel (`POST /api/upload/cancel/:jobId`): backend marks the job `cancelled` in memory first, aborts the in-flight fetch, then calls AI `POST /cancel` (404 from the AI is tolerated). The AI deletes its partial output, including its cached preview frames for that job; the backend likewise deletes `PREVIEW_DIR/{jobId}` on every terminal state.
5. `GET /api/upload/result/:jobId` (completed only) returns metadata; `GET /api/upload/stream/:jobId` streams the enhanced video with HTTP Range support (`…/original` streams the comparison video); `GET /api/upload/preview/:jobId/latest` (`Cache-Control: no-store`) and `GET /api/upload/preview/:jobId/:frameIndex` (public, `max-age=31536000, immutable`) serve preview JPEGs cache-through for known jobs — from disk when cached, fetched on demand from the AI while the job processes, 404 otherwise (unknown jobs are rejected without contacting the AI).
