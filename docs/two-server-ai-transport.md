...
1. App server (`10.10.248.133`, `upscale.cs.colman.ac.il`): Nginx reverse proxy,
   ...
   a strong `AI_INTERNAL_TOKEN`, and a bounded `MAX_FILE_SIZE_MB` (e.g. 500 —
...
- Recommended public upload cap is 500 MB — a weak proxy only: the AI loads all frames into RAM, so clip length (frame count), not file size, is the real memory driver, and no duration/frame-count guard exists yet.
