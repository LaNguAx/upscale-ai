...
3. **ComparisonPlayer** — `Products.tsx` keeps ONE `ComparisonPlayer.tsx` mounted across `processing → completed` (no component swap on completion). While processing it layers the pixel-aligned original/enhanced JPEG pair for the same `frameIndex` (`UPLOAD_PREVIEW_FRAME_ENDPOINT` + `…_ORIGINAL_FRAME_ENDPOINT` — frame-indexed URLs are immutable/cacheable, both images preloaded before swap, no `Date.now()` cache-busting), split by a pointer-dragged CSS `clip-path` divider. On completion it swaps (once the first video frame decodes) to the H.264 enhanced `<video>` (`UPLOAD_STREAM_ENDPOINT`) with the original comparison video (`UPLOAD_STREAM_ORIGINAL_ENDPOINT`, present when `jobResult.originalStreamUrl` is set) synced underneath, and custom controls: play/pause, seek, elapsed/total time, volume + mute (enhanced only), fullscreen. Whenever an original layer is unavailable the player degrades to enhanced-only and hides the divider. Still-frame previews during processing are not a live video stream; the final video only exists after `completed`.
