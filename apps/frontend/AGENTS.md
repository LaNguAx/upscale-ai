...
1. **Upload** — `VideoUploadForm.tsx` (drag-and-drop, MIME allowlist, 500 MB client-side sanity cap — the server's `MAX_FILE_SIZE_MB` is the source of truth, so the visible copy deliberately names no size). `uploadVideo` in `upscale.api.ts` uses a **custom XHR in `queryFn`** because fetch has no upload progress events — do not refactor it to a plain `builder.mutation`/fetch. FormData fields: `video` (file) only. Errors are parsed as RFC 7807 (`detail`/`title`); status **413** is special-cased with a friendly upload-limit message even when the body is non-JSON (a raw Nginx 413 HTML page), preferring the backend's `detail` (which names the exact cap) when parseable. The whole product flow is wrapped in `ProductErrorBoundary.tsx` (a small class component) so a render crash shows an alert + reset instead of a white page.
