...
- `src/upload/` — the product:
  ...
  - `ai-protocol.types.ts` — strict internal TS types for the (schema-less) backend↔AI NDJSON protocol: `AIHealthResponse`, `AITransferMode`, and the `AIProcessUpdate` discriminated union (`processing | completed | failed | cancelled`, `completed` carrying an optional `resultDownloadUrl`, `processing` carrying an optional `AIPreviewUpdate` preview object incl. optional `fps`/`stride` pacing metadata, `failed` carrying **either** a raw `error` **or** an `errorCode` + user-safe `message` pair).
...
3. **NDJSON consumption** — `ai-client.service.ts` parses the response body line-by-line into `AIProcessUpdate` values; `processing.service.ts` reacts: `failed` throws (job → `failed`) — when the line carries an `errorCode` recognised by `jobErrorCodeSchema` it throws an `AiJobError` holding that code, so the job is marked failed with the AI's user-safe `message` and the code is surfaced on `JobStatus`/`JobUpdate` as `errorCode`; an unrecognised or absent code falls back to the raw `error` text; `cancelled` marks the job cancelled; `processing` updates the percent; `completed` (after the remote download, if any) verifies the output file exists, best-effort acquires the `{jobId}_original.mp4` comparison video (remote: downloads `originalDownloadUrl`; path: picks it up from shared disk) via `setOriginalComparisonPath`, calls `setResultPath`, marks `completed` at 100. A stream that ends without a terminal line → `failed`. Malformed JSON → `failed`. The `AIProcessUpdate` shape lives in `ai-protocol.types.ts` — there is still no shared Zod schema for this internal protocol (full message shapes in `apps/ai/AGENTS.md`).
