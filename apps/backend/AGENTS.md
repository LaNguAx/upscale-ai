...
- `src/upload/` — the product:
  ...
  - `ai-protocol.types.ts` — strict internal TS types for the (schema-less) backend↔AI NDJSON protocol: `AIHealthResponse`, `AITransferMode`, and the `AIProcessUpdate` discriminated union (`processing | completed | failed | cancelled`, `completed` carrying an optional `resultDownloadUrl`).
