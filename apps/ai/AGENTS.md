...
- `baseline/vsr_inference.py` — the **only** inference module: `VSRInferenceEngine` (sliding-window frame loop, cancellation, progress callbacks), the `_FrameSource` reader/decode-fallback context manager, `InferenceCancelledError`, and `ResolutionTooHighError`. Keep inference in this one file, and keep it simple — the scheduling abstractions that used to live here were removed deliberately.
...
- `POST /process` (path mode) and `POST /process-upload` (remote mode, multipart `jobId` + `video`) both return `application/x-ndjson` `StreamingResponse`, one JSON object per line:
  ...
  - Failed (terminal): `{ "status": "failed", "jobId", "error": "<exception message>" }`.
    **Expected rejections carry a code instead of the raw exception**: a `ResolutionTooHighError` (input above `MAX_INPUT_HEIGHT`) emits `{ "status": "failed", "jobId", "errorCode": "INPUT_RESOLUTION_TOO_HIGH", "message": "<user-safe copy>" }` with **no `error` field** — the technical text ("Input resolution 1920x1080 exceeds…") goes to the server log only. Any other exception keeps the `error` form.
...
- Input taller than `MAX_INPUT_HEIGHT` is **rejected** (`ResolutionTooHighError`, a `RuntimeError`, so `server.py`'s existing handler turns it into a `failed` NDJSON line naming the resolution). There is no auto-downscale path — the video must be re-encoded smaller first.
- Inference resolution is the source resolution, rounded **down to even** in both axes (`_even`). This is the only resizing the engine does: `-pix_fmt yuv420p` cannot encode odd width, which otherwise makes the `{jobId}_original.mp4` comparison encode fail.
...
- **One forward pass per output frame**, each frame decoded at the centre of its own 15-frame window — identical to the original V3 implementation. The model returns an output for every frame in the window, but only the intended one is kept. This is deliberate: reusing the other outputs to cut passes was tried and removed, because it decodes two frames in three off-centre. Do not reintroduce it as a speed optimisation.
