...
Python FastAPI inference service for video super-resolution. Port 8000. PyTorch BasicVSR + SPyNet (V4 codec-degradation checkpoint by default, 4x scale, seq_len 15).
...
- `test_windowing.py` — unit tests for `baseline/windowing.py` (chunk scheduling, high-res policy, even dimensions). Same style: `python test_windowing.py`, no torch/GPU. Loads `windowing` as a top-level module so importing `baseline/__init__.py` (and therefore torch) is avoided.
...
- `baseline/vsr_inference.py` — `VSRInferenceEngine` (streaming frame loop, cancellation, progress callbacks) and the `_FrameSource` reader/decode-fallback context manager.
- `baseline/windowing.py` — pure, dependency-free scheduling and sizing helpers (`next_chunk`, `iter_chunks`, `target_size`, `even_dim`, `HighResPolicy`, `ResolutionTooHighError`). No torch/cv2 imports, so `test_windowing.py` runs anywhere — the same split `security.py` uses.
- `checkpoints/` — model weights, committed to the repo (~21 MB each, force-added past the `**/checkpoints/*.pth` gitignore rule): `vsr_model_v4_best_codec_psnr.pth` (V4 codec-degradation fine-tune, **the default**) and `vsr_model_best.pth` (V3, the original baseline — select it via `CHECKPOINT_PATH`). Without a checkpoint the server boots with `model_loaded: false` and `/process` returns 503.
...
- `POST /process` (path mode) and `POST /process-upload` (remote mode, multipart `jobId` + `video`) both return `application/x-ndjson` `StreamingResponse`, one JSON object per line:
  - Progress (emitted only when the integer percent changes; no `jobId`):
    ...
    When `PREVIEW_ENABLED` and a sampled preview JPEG was just written (frame 1, then every `PREVIEW_EVERY_N_FRAMES`th frame — its own cadence, independent of the percent-change dedup above), the same line carries an extra `preview` object: `{ "status": "processing", "progress": 42, "currentFrame": 210, "totalFrames": 500, "preview": { "frameIndex": 210, "width": 640, "height": 360, "downloadUrl": "/preview/{jobId}/210", "originalDownloadUrl": "/preview/{jobId}/210_in", "fps": 30.0, "stride": 2 } }`. The matching original (input) frame is written next to the enhanced one and referenced via `originalDownloadUrl` (omitted if that write failed). `fps` is the source video's frames/sec (probed by `server.py` with OpenCV before inference — the engine only returns fps *after* processing — with the same 30.0 fallback for broken metadata) and `stride` is `PREVIEW_EVERY_N_FRAMES`; together they let the client pace flipbook playback at `fps / stride`. `preview` is only present when the enhanced JPEG was actually written for that frame — never base64-inlined, never on every line.
  - Completed (terminal): `{ "status": "completed", "jobId", "progress": 100, "totalFrames", "fileSize" }`. In remote mode this line also carries `"resultDownloadUrl": "/result/{jobId}"` and, when the comparison encode succeeded, `"originalDownloadUrl": "/result/{jobId}/original"`. Before this line is emitted, the raw output is re-encoded to browser-safe H.264/yuv420p MP4 (`_finalize_outputs`), **muxing in the original input's audio track** as AAC (`-map 1:a:0?` optional map — silent inputs still finalize; `-shortest` handles length mismatch), and a `{jobId}_original.mp4` comparison video is encoded from the input at the inference resolution (kept audio-free — it plays muted under the comparison divider) — both best-effort (a failed transcode logs a warning and degrades, never fails the job). After the line is queued, the job's `WORK_PREVIEW_DIR/{jobId}/` preview frames are deleted (useless once the final video exists; a backend preview fetch racing the cleanup just gets a 404).
...
- Reads video with OpenCV via the `_FrameSource` context manager; if decoding fails, transcodes via **ffmpeg** (must be on PATH) to a temp H.264 file and retries. The capture, the writer, and the temp fallback file are released on every exit path, including cancellation.
- Input taller than `MAX_INPUT_HEIGHT` is handled per `HIGH_RES_POLICY` — **rejected by default** (`ResolutionTooHighError`, a `RuntimeError`, so `server.py`'s existing handler turns it into a `failed` NDJSON line naming the resolution). `downscale` restores the old auto-resize behaviour, now logged rather than silent; `native` skips resizing entirely (4x output — VRAM-hungry).
- Inference resolution is always **even** in both axes. Odd dimensions are corrected even when no downscale is needed: `-pix_fmt yuv420p` cannot encode odd width, which previously made the `{jobId}_original.mp4` comparison encode fail for sources like 1920x1080 (→ 853x480).
- **Frames are streamed.** A `deque(maxlen=seq_len)` holds only the current 15-frame window, so RAM is O(seq_len) rather than O(video length). Because the deque always holds the most recent 15 frames, at EOF it already *is* the final window — no backward seek needed.
- The model returns a super-resolved output for **every** frame in the window it is given. `context_radius` (T, default 1) sets how many are kept per call: the middle `2T+1`. Each kept frame retains at least `half - T` (6 by default) frames of real temporal context on its thinner side, at roughly `2T+1`x fewer forward passes (T=1 ≈ 3x, T=3 ≈ 7x, T=7 ≈ 15x). `T=0` reproduces the original one-pass-per-frame behaviour exactly.
- Windows **shift** rather than pad at the true start and end, so the first and last frames get the identical window the original implementation gave them. Repeat-last-frame padding applies only to clips shorter than `seq_len`.
- Not knowing the frame count in advance costs at most **one extra forward pass** per video (the loop commits to a chunk before it can tell EOF is near); frame output is unaffected.
- The writer produces an OpenCV `mp4v` intermediate at the input FPS, 4x resolution, **video-only** — then `server.py` re-encodes it to H.264/yuv420p `+faststart` `.mp4` in place (browsers cannot decode mp4v), muxing the original input's audio back in as AAC, and encodes a `{jobId}_original.mp4` comparison video (`-an`) from the input at the inference (LR) resolution.
- `progress_callback(current, total)` fires after each frame, 1-indexed with no gaps; `server.py` dedupes to integer percent changes. `total` is a best-effort count (container metadata, else a `grab()`-only pre-pass) and never drops below the current frame, so progress cannot exceed 100%. The `frames` value returned at the end is always the exact decoded count.
...
Per-job `threading.Event` in `active_cancellations` (lock-guarded). `/cancel` sets the event; the inference loop checks it before writing **each individual output frame** (not once per chunk, so cancel latency does not depend on `context_radius`) and raises `InferenceCancelledError`, which deletes the partial output file, the `{jobId}_original.mp4` comparison video (if present), and that job's `WORK_PREVIEW_DIR/{jobId}/` directory (when preview generation was active), then emits the `cancelled` NDJSON line. Preview-directory cleanup (`_cleanup_previews`) runs on **every** terminal path — completed and failed as well as cancelled — so preview frames never outlive their job. Inference runs in a daemon thread; the async generator polls a `queue.Queue` every 30ms and streams lines until a `None` sentinel.
...
- Env vars: `CHECKPOINT_PATH` (`./checkpoints/vsr_model_v4_best_codec_psnr.pth`), `DEVICE` (auto `cuda`/`cpu`), `MAX_INPUT_HEIGHT` (`480`), `HIGH_RES_POLICY` (`reject` — taller input is refused; `downscale` resizes to `MAX_INPUT_HEIGHT`, `native` runs at full resolution; an unrecognised value logs a warning and falls back to `reject`), `HOST`/`PORT` (`0.0.0.0`/`8000`, only when run as `__main__`), `AI_INTERNAL_TOKEN` (default empty — required for auth in remote mode, must match the backend), `WORK_UPLOAD_DIR` (`../../storage/ai/uploads`), `WORK_RESULT_DIR` (`../../storage/ai/results`, both resolved relative to `apps/ai` and created at startup), `PREVIEW_ENABLED` (default `true` — master switch for sampling/writing preview JPEGs), `PREVIEW_EVERY_N_FRAMES` (`2` — the flipbook playback stride: a pair is written for frame 1 then every Nth frame, so 2 ≈ 15 fps playback at a 30 fps source; note a lower stride multiplies preview disk usage ~`15/N`× vs the old default of 15, ≈80 KB/pair, offset by every-terminal-state cleanup), `PREVIEW_MAX_WIDTH` (`640`, aspect-preserving downscale, never upscales), `PREVIEW_JPEG_QUALITY` (`80`), `WORK_PREVIEW_DIR` (`../../storage/ai/previews`, resolved relative to `apps/ai` and created at startup). See `.env.development.example` / `.env.production.example`.
- External runtime deps not in `requirements.txt`: **ffmpeg on PATH is required** (H.264 finalize of every result incl. the AAC audio mux + original comparison encode + decode fallback; without it results stay mp4v, silent, and not browser-playable), CUDA optional (~8 GB VRAM recommended for 720p output).
